                  ...
                  <tbody>
                      ...
                      if (!empty($data_query)) {
                          ...
                          foreach ($data_query as $value) {
                              ...
                              echo '<td>' . ($value->type==1?'Pegawai':'Admin') . '</td>';
                              if($value->type!=0){
                               echo '<td>' . ($value->status==1?
                                    '<a href='.  
                                     base_url().$module.'/status_deaktiv/'.$value->id.'>'
                                   . '<button class="btn-danger btn">Non Aktifkan</button></a>':
                                   '<a href='.  
                                     base_url().$module.'/status_aktiv/'.$value->id.'>'
                                   . '<button class="btn-green btn">Aktifkan</button></a>') 
                                   . '</td>';
                              }  else {
                                  echo '<td>'.($value->status==1?'Aktiv':'Non Aktiv').'</td>'; 
                              }
                              ...
                              echo ($this->session->userdata('type')!=$value->type and $this->session->userdata('type')==0 )? ' <a href="#" data-id="' . $value->id . '" class="hapus_ajax">'
         ...
         <form id="form-tambah"  method="post" class="form-horizontal" >
            <div class="modal-body">
               ...
               <div class="form-group">
                  ...
                  <div class="col-sm-4">                     
                     <input id="nama" type="text" class="form-control" name="name" value="">
                  </div>
               ...
               <div class="form-group">
                  ...
                  <div id="username-error" class="error_show">Data tidak Boleh Kosong</div>
               </div>
               <div class="form-group">
                  ...
                  <div id="password-error" class="error_show">Data tidak Boleh Kosong</div>
               </div>
               <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#username').on('input', function () {
            ...
            var is_name = input.val();
            if (is_name) {
                $("#username-error").removeClass('error_show').addClass('error');
            } else {
                $("#username-error").removeClass('error').addClass('error_show');
            }
        });
        $('#password').on('input', function () {
            ...
            var is_name = input.val();
            if (is_name) {
                $("#password-error").removeClass('error_show').addClass('error');
            } else {
                $("#password-error").removeClass('error').addClass('error_show');
            }
        });
